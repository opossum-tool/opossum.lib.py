...
def convert_to_opossum(opossum_file: OpossumFileContent) -> Opossum:
    opossum = Opossum(
        scan_results=convert_to_opossum_scan_results(opossum_file.input_file),
        review_results=opossum_file.output_file,
    )
    return opossum


def convert_to_opossum_scan_results(
    opossum_information: OpossumInformation,
) -> ScanResults:
    resources, used_attribution_ids = convert_to_opossum_model_resource_tree(
    ...
    frequent_licenses = (
        ...
        and convert_frequent_licenses_to_model_frequent_licenses(
            opossum_information.frequent_licenses
        )
    )
    ...
    external_attribution_sources = {
        name: convert_external_attribution_source(attribution_source)
        for name, attribution_source in file_attribution_sources.items()
    }

    attribution_with_id = convert_to_attribution_with_id(
        opossum_information.external_attributions
    )
    return ScanResults(
        metadata=convert_to_opossum_model_metadata(opossum_information.metadata),
        resources=resources,
        attribution_breakpoints=deepcopy(opossum_information.attribution_breakpoints),
        ...
        unassigned_attributions=get_unassigned_attributions(
            used_attribution_ids, opossum_information.external_attributions
        ),
    )


def get_unassigned_attributions(
    ...
    unused_attributions = [
        convert_package(external_attributions[id]) for id in unused_attributions_ids
    ]
    return unused_attributions


def convert_external_attribution_source(
...
def convert_frequent_licenses_to_model_frequent_licenses(
    frequent_licenses_infile: list[opossum_lib.opossum.opossum_file.FrequentLicense],
) -> list[FrequentLicense]:
    frequent_licenses: list[FrequentLicense] = [
        convert_frequent_license(license) for license in frequent_licenses_infile
    ]
    return frequent_licenses


def convert_to_opossum_model_metadata(
    infile_metadata: opossum_lib.opossum.opossum_file.Metadata,
) -> Metadata:
    return Metadata(**infile_metadata.model_dump())


def convert_to_opossum_model_resource_tree(
    resources: opossum_lib.opossum.opossum_file.ResourceInFile,
    external_attributions: dict[
        opossum_lib.opossum.opossum_file.OpossumPackageIdentifier,
        opossum_lib.opossum.opossum_file.OpossumPackage,
    ],
    resources_to_attributions: dict[
        opossum_lib.opossum.opossum_file.ResourcePath,
        list[opossum_lib.opossum.opossum_file.OpossumPackageIdentifier],
    ],
    ...
    def get_applicable_attributions(
        ...
        if current_path_as_string in resources_to_attributions:
            ...
            attributions = [
                convert_package(external_attributions[id]) for id in attribution_ids
            ]
...
def convert_to_attribution_with_id(
    external_attributions: dict[
        opossum_lib.opossum.opossum_file.OpossumPackageIdentifier,
        opossum_lib.opossum.opossum_file.OpossumPackage,
    ],
    ...
    return {
        convert_package(package): package_identifier
        for package_identifier, package in external_attributions.items()
    }


def convert_frequent_license(
    infile_frequent_license: opossum_lib.opossum.opossum_file.FrequentLicense,
) -> FrequentLicense:
...
def convert_package(
    infile_package: opossum_lib.opossum.opossum_file.OpossumPackage,
) -> OpossumPackage:
    pass
    return OpossumPackage(
        source=convert_source(infile_package.source),
...
def convert_source(
    infile_source_info: opossum_lib.opossum.opossum_file.SourceInfo,
) -> SourceInfo:
    return SourceInfo(
